...
export function CartProvider({children}: CartProviderProps) {
    ...
    function updateSnackQuantity( snack: Snack, newQuantity: number) {
        ...
        const snackExistenInCart = cart.find((item) => item.id === snack.id && item.snack === 
        snack.snack)

        if (!snackExistenInCart) return 

        const newCart = cart.map((item) => {
            if (item.id === snackExistenInCart.id && item.snack === snackExistenInCart.
            snack) {
            return {
                    ...item,
